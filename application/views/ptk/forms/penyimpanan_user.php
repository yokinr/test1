				...
				if($penyimpanan){
					...
					<ul class="list-group">
						<?php foreach ($penyimpanan as $key1 => $value1): ?>
							...
							<li class="tombol-1 list-group-item d-flex justify-content-between align-items-center" id="tampil_file_upload" data-id="<?= $dataID ?>" md-v="modal-xl"><?= $value1->nama_file ?><span class="badge text-dark"><?= "<i class='fas fa-calendar-alt text-dark'></i> ".date('d-m-Y H:i:s', strtotime($value1->tanggal)) ?></span></li>
						<?php endforeach ?>
					</ul>
					<?php
				}
...
<script type="text/javascript">
	$('.modal').on('click', '.tombol-1', function(){
		$('.tombol-1').removeClass('bg-primary text-light');
