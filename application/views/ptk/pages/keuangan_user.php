						...
						foreach ($mapping as $key => $value) {
							...
							foreach ($value as $key1 => $value1) {
								...
								if($detail_iuran->jenis_iuran=='2'){
									$cek_pembayaran_perItem = $this->m_keuangan->cek_pembayaran_perItem($user_id, $value1->uniq_iuran);
									...
									<tr>
										...
										if($cek_pembayaran_perItem['tanggal']){
											...
											if($cek_pembayaran_perItem['nominal'] < $detail_iuran->nominal){
												...
												<td><button class="btn btn-link btn-sm tombol-1" id="iuran_tambah_pembayaran_perItem" data-id="<?= $dataID ?>"><i class="fas fa-plus-circle"></i></button> <button class="btn btn-link btn-sm text-danger tombol-1" id="iuran_hapus_pembayaran_perItem" data-id="<?= $dataID ?>"><i class="fas fa-trash"></i></button></td>
												<?php
											}else{
												?>
												<td class="text-right text-primary">Rp. <?= number_format($cek_pembayaran_perItem['nominal'],0,',','.') ?></td>
												<td><button class="btn btn-link btn-sm text-danger tombol-1" id="iuran_hapus_pembayaran_perItem" data-id="<?= $dataID ?>"><i class="fas fa-trash"></i></button></td>
												<?php
											}
										}else{
											?>
						...
						<div class="col border p-2 m-2">
							...
							<div class="table-responsive">
								<table class="table">
									...
									if($semester=='1'){
										for ($i=7; $i < 13; $i++) { 
											$cek_pembayaran_rutin = $this->m_keuangan->cek_pembayaran_rutin($user_id, $value1->uniq_iuran, $i, $value1->semester_id);
											$sisanominal = $detail_iuran->nominal - $cek_pembayaran_rutin->nominal;
											$object1 = array(
												'peserta_didik_id' => $user_id,
												'uniq_iuran' => $value1->uniq_iuran,
												'bulan_rutin' => $i,
											...
											<tr>
												...
												if ($cek_pembayaran_rutin->tanggal){
													...
													<td><?= number_format($cek_pembayaran_rutin->nominal,0,',','.') ?></td>
													<?php
												}else{
													?>
													<td>-</td>
									...
									if($semester=='2'){
										for ($j=1; $j < 7; $j++) { 
											$cek_pembayaran_rutin = $this->m_keuangan->cek_pembayaran_rutin($user_id, $value1->uniq_iuran, $j, $value1->semester_id);
											$sisanominal = $detail_iuran->nominal - $cek_pembayaran_rutin->nominal;
											$object2 = array(
												'peserta_didik_id' => $user_id,
												'uniq_iuran' => $value1->uniq_iuran,
